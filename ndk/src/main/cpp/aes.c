...
//            arr[j] = state[j][i];//0.4.8.12
//
//        (*state)[0][i] = Mul_02[arr[0]] ^ Mul_03[arr[1]] ^ arr[2] ^ arr[3];
//        (*state)[1][i] = arr[0] ^ Mul_02[arr[1]] ^ Mul_03[arr[2]] ^ arr[3];
//        (*state)[2][i] = arr[0] ^ arr[1] ^ Mul_02[arr[2]] ^ Mul_03[arr[3]];
//        (*state)[3][i] = Mul_03[arr[0]] ^ arr[1] ^ arr[2] ^ Mul_02[arr[3]];
//    }
//}
...
//            arr[j] = state[j][i];//0.4.8.12
//
//        (*state)[0][i] = Mul_0e[arr[0]] ^ Mul_0b[arr[1]] ^ Mul_0d[arr[2]] ^ Mul_09[arr[3]];
//        (*state)[1][i] = Mul_09[arr[0]] ^ Mul_0e[arr[1]] ^ Mul_0b[arr[2]] ^ Mul_0d[arr[3]];
//        (*state)[2][i] = Mul_0d[arr[0]] ^ Mul_09[arr[1]] ^ Mul_0e[arr[2]] ^ Mul_0b[arr[3]];
//        (*state)[3][i] = Mul_0b[arr[0]] ^ Mul_0d[arr[1]] ^ Mul_09[arr[2]] ^ Mul_0e[arr[3]];
//    }
//}
...
void
AES_ECB_encrypt(const uint8_t *input, const uint8_t *key, uint8_t *output, const uint32_t length) {
    int count = length / BLOCKLEN;
    //开始分段加密
...
void
AES_ECB_decrypt(const uint8_t *input, const uint8_t *key, uint8_t *output, const uint32_t length) {
    int i;
...
static void XorWithIv(uint8_t *buf, uint8_t *Iv) {
    uint8_t i;
    for (i = 0; i <
                BLOCKLEN; ++i) //WAS for(i = 0; i < KEYLEN; ++i) but the block in AES is always 128bit so 16 bytes!
    {
        buf[i] ^= Iv[i];
    }
}

void AES_CBC_encrypt_buffer(uint8_t *output, uint8_t *input, uint32_t length, const uint8_t *key,
                            const uint8_t *iv) {
    uintptr_t i;
...
void AES_CTR_xcrypt_buffer(uint8_t *output, uint8_t *input, uint32_t length, const uint8_t *key,
                           const uint8_t *nonce) {
